  ...
  for (const auto& directory : s_directories_to_create)
  {
    ...
    if (result != FS::ResultCode::Success && result != FS::ResultCode::AlreadyExists)
    {
      ERROR_LOG_FMT(IOS_ES, "Failed to create {}: error {}", directory.path,
                    static_cast<int>(FS::ConvertResult(result)));
    }
                                     ...
                                     const std::vector<u8>& cert_chain, u32* issuer_handle_out)
{
  ...
  if (ret != IPC_SUCCESS)
  {
    ERROR_LOG_FMT(IOS_ES, "VerifyContainer: IOSC_ImportCertificate(ca) failed with error {}", static_cast<int>(ret));
    return ret;
  }
  ...
  if (ret != IPC_SUCCESS)
  {
    ERROR_LOG_FMT(IOS_ES, "VerifyContainer: IOSC_VerifyPublicKeySign failed with error {}", static_cast<int>(ret));
    return ret;
  }
  ...
  if (mode == VerifyMode::UpdateCertStore)
  {
    ...
    if (ret != IPC_SUCCESS)
      ERROR_LOG_FMT(IOS_ES, "VerifyContainer: Writing the CA cert failed with return code {}", static_cast<int>(ret));
  }
