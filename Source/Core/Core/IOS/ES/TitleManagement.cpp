  ...
  if (type == TicketImportType::PossiblyPersonalised && ticket_device_id != 0)
  {
    ...
    if (ret < 0)
    {
      ERROR_LOG_FMT(IOS_ES, "ImportTicket: Failed to unpersonalise ticket for {:016x} ({})",
                    ticket.GetTitleId(), static_cast<int>(ret));
      return ret;
    }
  }
                               ...
                               u64 caller_title_id, u32 caller_title_flags)
{
  ...
  if (ret != IPC_SUCCESS)
  {
    ERROR_LOG_FMT(IOS_ES, "ImportTmd: VerifyContainer failed with error {}", static_cast<int>(ret));
    return ret;
  }
  ...
  if (ret != IPC_SUCCESS)
  {
    ERROR_LOG_FMT(IOS_ES, "ImportTmd: InitBackupKey failed with error {}", static_cast<int>(ret));
    return ret;
  }
