...
void Renderer::BindBackbuffer(const ClearColor& clear_color)
{
  ...
  if (res != VK_SUCCESS)
  {
    ...
    {
      ERROR_LOG_FMT(VIDEO, "Unknown present error {:#010X}, please report.", static_cast<unsigned int>(res));
      m_swap_chain->RecreateSwapChain();
    }

    res = m_swap_chain->AcquireNextImage();
    if (res != VK_SUCCESS)
      PanicAlertFmt("Failed to grab image from swap chain: {:#010X}", static_cast<unsigned int>(res));
  }
